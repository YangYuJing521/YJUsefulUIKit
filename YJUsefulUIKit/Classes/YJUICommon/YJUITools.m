...
+ (void)viewWithCorner:(UIView *)view cornerRadius:(CGFloat)radius lineWidth:(CGFloat)lineWidth lineColor:(UIColor *)lineColor rectType:(UIRectCorner)type {
    ...
    UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRoundedRect:view.bounds byRoundingCorners:type cornerRadii:CGSizeMake(radius, radius)];
...
+ (nonnull UIImage *)PlaceHolderImageWithImage:(NSString *)imageName width:(CGFloat)width height:(CGFloat)height{
    @autoreleasepool{
        NSString *pngName = [NSString stringWithFormat:@"PlaceHolder%.0f%.0f.png",width,height];
        NSString *cachePath = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES).lastObject;
        NSString *filePach = [cachePath stringByAppendingPathComponent:pngName];
        BOOL isExit =[[NSFileManager defaultManager] fileExistsAtPath:filePach];
        if (isExit) {
            ...
            placholder.image = [UIImage imageNamed:imageName inBundle:[YJPodUtil bundleForPod:@"UseFulUIKit"] compatibleWithTraitCollection:nil];
            UIImage *placeHolderImage = [YJUITools createImageFromView:placholder];
